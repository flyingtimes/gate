...
// 任何你期望执行的cmd命令，ls都可以
let cmdStr = appRootDir + '/bin/nebula'
let etcStr = appRootDir + '/etc/config-mac-at-home.yaml'
...
function runExec() {
  ...
  workerProcess = sudo.exec(cmdStr+' -config '+etcStr, options,
...
const createWindow = () => {
  ...
  var iconpath = path.join(__dirname, 'resource/icon.png')
